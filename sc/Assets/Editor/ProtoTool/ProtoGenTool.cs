...
public class ProtoGenTool : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("选择proto文件", GUILayout.Height(20)))
        {
        ...
        EditorGUILayout.BeginHorizontal();
        GUILayout.Label("输入目录:");
        if (string.IsNullOrEmpty(outProtoPath))
        {
            outProtoPath = EditorPrefs.GetString("ProtoGenTool_outProtoPath", "");
        }
        outProtoPath = EditorGUILayout.TextField(outProtoPath);
        if (string.IsNullOrEmpty(outProtoPath)) outProtoPath = "默认路径：Assets/Scripts/ProtoMessage/";
        if (GUILayout.Button("选择输出目录", GUILayout.Height(20)))
        {
            if (string.IsNullOrEmpty(outProtoPath)) outProtoPath = Application.dataPath;
            outProtoPath = EditorUtility.OpenFolderPanel("选择输出目录", outProtoPath, "define");
            EditorPrefs.SetString("ProtoGenTool_outProtoPath", outProtoPath);
        }
        EditorGUILayout.EndHorizontal();
    ...
    private void OnGenBtnClickEnable(string fileName)
    {
        ...
        if (!string.IsNullOrEmpty(outProtoPath))
        {
            hotfixMessageCodePath = outProtoPath;
        }
