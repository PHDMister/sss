...
public class SceneLoadManager : MonoBehaviour
{
    ...
    public void Load(int level)
    {
        ...
        toProgress = 0;
        m_SceneLevel = level;
        ManageMentClass.DataManagerClass.SceneID = 9;
    ...
    //赛车场景
    protected void GoToDoubleracing()
    {

        StartCoroutine(InitDoubleracing());
    }
    protected IEnumerator InitDoubleracing()
    {
        yield return null;
        UIManager.GetInstance().CloseAllShowPanel();
        Debug.Log("输出一下这里正在重复打开面板");
        UIManager.GetInstance().ShowUIForms(FormConst.HOMEMAINPANEL);
    }
