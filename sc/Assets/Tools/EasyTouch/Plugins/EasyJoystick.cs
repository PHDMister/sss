...
public class EasyJoystick : MonoBehaviour
{
    ...
    [SerializeField]
    private bool restrictArea = true;
    /// <summary>
    ...
    //======================自定义图===========================
    [SerializeField]
    private float customRadius = 100f;
    ...
    [SerializeField] public Vector2 CustomOffset = Vector2.zero;
    #endregion
    ...
    void OnGUI()
    {
        ...
        if (enable)
        {
                ...
                || (dynamicJoystick && Application.isEditor && !Application.isPlaying))
            {
                ...
                ZoneCenter = GUIUtility.GUIToScreenPoint(areaRect.center);
                Rect touchRect1 = new Rect(anchorPosition.x + joystickCenter.x + (joystickTouch.x - touchSize),
                    anchorPosition.y + joystickCenter.y - (joystickTouch.y + touchSize), touchSize * 2, touchSize * 2);
                TouchCenter = GUIUtility.GUIToScreenPoint(touchRect1.center);
                Vector3 dir = TouchCenter - ZoneCenter;
                float angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg;
                ZoneAngle = angle + 90;

                GUIUtility.RotateAroundPivot(ZoneAngle, ZoneCenter);
                GUI.DrawTexture(areaRect, areaTexture, ScaleMode.StretchToFill, true);
                if (CustomTexture != null) GUI.DrawTexture(CustomArea, CustomTexture, ScaleMode.StretchToFill, true);
                ...
                || (dynamicJoystick && Application.isEditor && !Application.isPlaying))
            {
                ...
                Rect touchRect2 = new Rect(anchorPosition.x + joystickCenter.x + (joystickTouch.x - touchSize), 
                    anchorPosition.y + joystickCenter.y - (joystickTouch.y + touchSize), 
                    touchSize * 2, touchSize * 2);

                matrix4X4 = GUI.matrix;
                //TouchCenter = GUIUtility.GUIToScreenPoint(touchRect.center);
                //Vector3 dir = TouchCenter - ZoneCenter;
                //float angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg;
                //ZoneAngle = angle + 90;
                GUIUtility.RotateAroundPivot(ZoneAngle, TouchCenter);
                GUI.DrawTexture(touchRect2, touchTexture, ScaleMode.ScaleToFit, true);
                GUI.matrix = matrix4X4;
            }
    ...
    void ComputeJoystickAnchor(JoystickAnchor anchor)
    {
        ...
        CustomArea = new Rect(anchorPosition.x + joystickCenter.x - customRadius + CustomOffset.x, anchorPosition.y + joystickCenter.y - customRadius + CustomOffset.y, customRadius * 2, customRadius * 2);
    }
    #endregion
            ...
            if (isActivated)
            {
                ...
                if (gesture.fingerIndex == joystickIndex)
                {
                    if (((gesture.position - center).sqrMagnitude < (zoneRadius * VirtualScreen.xRatio) * (zoneRadius * VirtualScreen.xRatio) && resetFingerExit) || !resetFingerExit)
                    {
                        ...
                        showZone = true;
                        showDeadZone = false;
                        //bool bInArea = InterfaceHelper.IsInArea();
                        //Debug.Log("bInArea = " + bInArea);
                        //if(!bInArea)
                        //                  {
