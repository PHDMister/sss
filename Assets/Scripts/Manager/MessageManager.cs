...
public class MessageManager : MonoBehaviour
{
    ...
    public void SendTreasureRecordReq(uint page, uint page_size, ulong user_id, TreasureRecordListRespCallBack callback)
    {
    ...
    public void RequestGetPersonData(Action callBack, ulong userId = 0)
    {
        StartCoroutine(RequestGetPersonDataIE(callBack, userId));
    }
    IEnumerator RequestGetPersonDataIE(Action callBack, ulong userId)
    {
        HttpRequest httpRequest = new HttpRequest();
        Debug.Log("???????Use??ID????? " + userId);
        var data = new JObject
        {
            ["user_id"] = userId
        };
        StartCoroutine(httpRequest.PostRequest(ManageMentClass.ServerInterFaceClass.GetPersonData, ManageMentClass.DataManagerClass.tokenValue_Game, data.ToString()));
        while (!httpRequest.isComPlete)
        {
            yield return null;
        }
