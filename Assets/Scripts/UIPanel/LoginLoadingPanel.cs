...
public class LoginLoadingPanel : BaseUIForm
{
    ...
    void OnWsNetFirstOpen(KeyValuesUpdate kv)
    {
        ...
        WebSocketAgent.SendMsg((uint)MessageId.Types.Enum.Register, registerReq, (code, bytes) =>
        {
            ...
            if (registerResp.StatusCode == 0)
            {
                ...
                TreasureModel.Instance.TreasureEfficiencyTime = registerResp.TreasureEfficiencyTime;
                TreasureModel.Instance.TreasureEfficiencyBuff = registerResp.TreasureEfficiencyBuff;
                Debug.Log($"WebSocketConnect  RegisterResp userId:{registerResp.UserId}");
        ...
        MessageManager.GetInstance().LoginHDFun(() =>
        {
            ...
            else
            {
                if (SelfConfigData.testLandId == "")
                {
                    GetLandIDActionFun(() =>
                    {
                        Debug.Log($"输出一下获取道德LandID：  {ManageMentClass.DataManagerClass.landId} </color>");
                        //µÇÂ¼³É¹¦µÄ·½·¨
                        //ÅÐ¶ÏÈ¥ÄÄ¸ö³¡¾°
                        SelectToWhereSceneFun();
    ...
    /// 去救助站的方法
    /// </summary>
    void ToShelterSceneFun()
    {
        MessageManager.GetInstance().RequestRoomList(ManageMentClass.DataManagerClass.landId, () =>
        {
            UIManager.GetInstance().ShowUIForms(FormConst.PETDENSLOADING);
            SendMessage("TransferLoading", "SceneTransfer", (int)LoadSceneType.ShelterScene);
            CloseUIForm();
        });
    }
    ...
    /// Ç°ÍùÍÚ±¦
    /// </summary>
    void GoToTreasureDigging()
    {
        UIManager.GetInstance().ShowUIForms(FormConst.PETDENSLOADING);
        SendMessage("TransferLoading", "SceneTransfer", (int)LoadSceneType.TreasureDigging);
        CloseUIForm();
    }
    void GetLandIDActionFun(Action CallBack)
    {
        if (SelfConfigData.testLandId == "")
        {
            MessageManager.GetInstance().GetLandIDListFun(SelfConfigData.AreaIDA, (jo) =>
             {
                 Debug.Log("Êä³öÒ»ÏÂ¸öÈËÓµÓÐµÄµÄÍÁµØÁÐ±í£º " + jo["data"]["my_land_list"]);

                 if (jo["data"]["my_land_list"].ToString() == "")
                 {
                     Debug.Log("AÇøµÄÍÁµØÎª¿Õ");
