...
public class PlayerControllerImp : MonoBehaviour
{
    ...
    {
        if (UserInfo != null && playerItem != null)
        {
            if (UserInfo.State == 4 && !playerItem.IsPlaying("W_Idle"))
                playerItem.SetAnimator("W_Idle");
            else if (UserInfo.State == 5 && !playerItem.IsPlaying("W_Mine"))
                playerItem.SetAnimator("W_Mine");
        }
    }
