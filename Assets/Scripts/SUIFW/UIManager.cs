...
namespace UIFW
{
    public class UIManager : MonoBehaviour
    {
        ...
        //顶层覆盖
        private Transform _TraTop = null;
        //UI管理脚本的节点
        private Transform _TraUIScripts = null;

        //RT根节点
        private Transform _TraRTTransform = null;
        //挂载模型节点
        private Transform _TraModelTransform = null;
        private Transform _TranModeCameraTrans = null;
        ...
        public void Awake()
        {
            ...
            _TraTop = UnityHelper.FindTheChildNode(_TraCanvasTransfrom.gameObject, SysDefine.SYS_TOP_NODE);
            _TraUIScripts = UnityHelper.FindTheChildNode(_TraCanvasTransfrom.gameObject, SysDefine.SYS_SCRIPTMANAGER_NODE);
            //RT节点
            _TraRTTransform = GameObject.FindGameObjectWithTag(SysDefine.SYS_TAG_RT).transform;
            _TraModelTransform = UnityHelper.FindTheChildNode(_TraRTTransform.gameObject, SysDefine.SYS_MODEL_NODE);
            _TranModeCameraTrans = UnityHelper.FindTheChildNode(_TraRTTransform.gameObject, SysDefine.SYS_CAMERA_NODE);
            //把本脚本作为“根UI窗体”的子节点。
            this.gameObject.transform.SetParent(_TraUIScripts, false);
            //"根UI窗体"在场景转换的时候，不允许销毁
            DontDestroyOnLoad(_TraCanvasTransfrom);
            DontDestroyOnLoad(_TraRTTransform);
            //初始化“UI窗体预设”路径数据
            //InitUIFormsPathData();
        }
        /// <summary>
        ...
        private void InitRT()
        {
            ResourcesMgr.GetInstance().LoadAsset(SysDefine.SYS_PATH_RT, false);
        }
        /// <summary>
        /// 根据UI窗体的名称，加载到“所有UI窗体”缓存集合中
	    ...
	    private BaseUIForm LoadUIForm(string uiFormName)
        {
            ...
            //不再读UIFormsConfigInfo.json下的配置路径直接读UIPrefabs目录下预设文件
            strUIFormPaths = string.Format("{0}/{1}", SysDefine.SYS_PATH_UIFORMS, uiFormName);
            //根据“UI窗体名称”，加载“预设克隆体”
            ...
            if (_TraCanvasTransfrom != null && goCloneUIPrefabs != null)
            {
                ...
                switch (baseUiForm.CurrentUIType.UIForms_Type)
                {
                    ...
                    case UIFormType.PopUp:                  //弹出窗体节点
                        ...
                        break;
                    case UIFormType.Top:                  //顶层覆盖
                        goCloneUIPrefabs.transform.SetParent(_TraTop, false);
                        break;
                    default:
                        break;
                }
        ...
        public BaseUIForm GetUIForm(string strUIName)
        {
            BaseUIForm baseUiForm;
            _DicALLUIForms.TryGetValue(strUIName, out baseUiForm);
            return baseUiForm;
        }
        public T GetUIForm<T>(string strUIName) where T : BaseUIForm
        {
            BaseUIForm baseUiForm;
            _DicALLUIForms.TryGetValue(strUIName, out baseUiForm);
        ...
        public Transform GetModelTrans()
        {
            return _TraModelTransform;
        }
        public Transform GetModelCameraTrans()
        {
            return _TranModeCameraTrans;
        }

        public bool IsOpend(string uiFormName)
        {
            ...
            bool bHave = (baseUiForm != null);
            bool bTop = false;
            if (_StaCurrentUIForms.Count > 0)
            {
                BaseUIForm topUIForm = _StaCurrentUIForms.Peek();
                bTop = topUIForm.name.Contains(uiFormName);
            }
            return bHave || bTop;
        }

        public bool IsOpend<T>(string uiFormName, out T ui) where T : BaseUIForm
        {
            ui = null;
            if (IsOpend(uiFormName))
            {
                ui = GetUIForm(uiFormName) as T;
                return true;
            }
            return false;
        }
        ...
        /// 隐藏所有正显示的面板
        /// </summary>
        public void CloseAllShowPanel()
        {
            foreach (string baseUI in _DicCurrentShowUIForms.Keys)
            {
                _DicCurrentShowUIForms[baseUI].Hiding();
            }
        ...
        public Transform GetTopLayerTrans()
        {
            return _TraTop;
        }
    }
