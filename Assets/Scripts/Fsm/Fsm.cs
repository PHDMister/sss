...
public class Fsm<T> : FsmBase, IFsm<T> where T : class
{
        ...
        foreach (FsmState<T> state in states)
        {
            ...
            if (fsm.m_States.ContainsKey(stateType))
            {
                throw new Exception(string.Format("FSM '{0}' state '{1}' is already exist.",
                    new TypeNamePair(typeof(T), name).ToString(), stateType));
            }
    ...
    public void Start<TState>() where TState : FsmState<T>
    {
        ...
        if (state == null)
        {
            throw new Exception(string.Format("FSM '{0}' can not start state '{1}' which is not exist.",
                new TypeNamePair(typeof(T), Name).ToString(), typeof(TState).FullName));
        }
    ...
    public void Start(Type stateType)
    {
        ...
        {
            throw new Exception(string.Format("FSM '{0}' can not start state '{1}' which is not exist.",
                new TypeNamePair(typeof(T), Name).ToString(), stateType.FullName));
        }
