...
public class RainbowBeachShareShells : BaseUIForm
{
    ...
    const string temp = "<color=#C8A000>{0}-{1}</color>【<color=#00BBB7>{2}</color>】发贝壳啦！";
    ...
    protected void OnAllRoomHandle(GameObject go, int index)
    {
        ...
        string sceneName = GetSceneString(data.SceneId);
        context.text = string.Format(temp, sceneName, data.RoomId, name);
    ...
    protected void OnCurRoomHandle(GameObject go, int index)
    {
        ...
        string sceneName = GetSceneString(data.SceneId);
        context.text = string.Format(temp, sceneName, data.RoomId, TextTools.setCutAddString(data.OwnerName, 8, "..."));
    ...
    protected string GetSceneString(uint sceneId)
    {
        if (sceneId == (uint)LoadSceneType.RainbowBeach)
            return "彩虹沙滩";
        if (sceneId == (uint)LoadSceneType.ShenMiHaiWan)
            return "神秘海湾";
        if (sceneId == (uint)LoadSceneType.HaiDiXingKong)
            return "海底星空";
