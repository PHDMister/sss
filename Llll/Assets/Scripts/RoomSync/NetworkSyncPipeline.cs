...
public class OtherChangeSkinPipeline : SyncCmdPipeline<OtherChangeSkinPush>
{
    ...
    public override void OnUpdate(float time)
    {
        ...
        {
            ManageMentClass.CurSyncPlayerController.CreateUserInfo(CurMsg.FromUserId, CurMsg.AvatarIds);
            ManageMentClass.CurSyncPlayerController.ReproduceUserState(CurMsg.FromUserId);
        }
...
/// 房间内角色状态同步
/// </summary>
public class SyncRainbowPlayerStatePipeline : SyncCmdPipeline<RoomRewardPush>
{
    protected float OtherTime;
    protected float DelayTime;
    protected string AnimName = "Shovel";
    private const string ShovelShellsAnim = "PickingShells";
    protected int waitOneFrame;
    protected GameObject Temp;
    protected string rod;

    public SyncRainbowPlayerStatePipeline(Action<PlayerControllerImp.SyncPipelineState> onSetState
        , MoveControllerImp imp, PlayerItem pItem)
        : base(onSetState, imp, pItem)
    {
        IsOverSetLastIndex = false;
    ...
    public override void Push(IMessage msg)
    {
        MsgList.Add(msg as RoomRewardPush);
        MsgList.Sort((a, b) => b.Index.CompareTo(a.Index));
    }
    public override bool CheckMsgIndex(uint lastIndex)
    {
        ...
        for (int i = MsgList.Count - 1; i >= 0; i--)
        {
            RoomRewardPush userState = MsgList[i];
            if (userState.Index <= lastIndex)
                MsgList.RemoveAt(i);
            //客户端重启过下标重置了 执行过的下标只会大于等于lastIndex
            else if (lastIndex < ExeIndex)
    ...
    private void RainbowUpdate(float time)
    {
        if (SyncProcesState == PlayerControllerImp.SyncProcesState.Idle)
        {
            if (!MoveController.IsIdle) return;
            PlayerItem.SetAnimator("Idle");
            SubModel subModel = Singleton<RainbowBeachController>.Instance.GetSubModel(CurMsg.ToolId, PlayerItem);
            subModel.SetActive(true);
            subModel.Play(AnimName);

            //角色动画
            //PlayerItem.SelfAnimator.Update(0);
            PlayerItem.SetAnimator(ShovelShellsAnim);
            PlayerItem.SelfAnimator.Play(AnimName, 0, 0);

            DelayTime = PlayerItem.GetCurAnimLength(AnimName);
            SyncProcesState = PlayerControllerImp.SyncProcesState.MoveSpline;
        }
        if (SyncProcesState == PlayerControllerImp.SyncProcesState.MoveSpline)
        {
            DelayTime -= time;
            if (DelayTime < 0)
            {
    ...
    private void HaiwanUpdate(float time)
    {
        if (SyncProcesState == PlayerControllerImp.SyncProcesState.Idle)
        {
            if (!MoveController.IsIdle) return;
            rod = Singleton<RainbowIocnController>.Instance.GetFishingRod(CurMsg.ToolId);
            Temp = CurMsg.RewardType == 4
                ? Singleton<RainbowIocnController>.Instance.GetWasteModel()
                : Singleton<RainbowIocnController>.Instance.GetFishModel(CurMsg.RewardCollectionProp);
            Temp.SetActive(false);
            SubModel subModel = PlayerItem.GetSubModel(rod);
            subModel.SetActive(true);
            subModel.Play(rod);
            subModel.AddModel("FishPoint", Temp);
            //角色动画
            OtherTime = 2;
            PlayerItem.SelfAnimator.Play(rod, 0, 0);
            DelayTime = PlayerItem.GetCurAnimLength(rod) - OtherTime * 0.85f;
            SyncProcesState = PlayerControllerImp.SyncProcesState.MoveStart;
        }
        if (SyncProcesState == PlayerControllerImp.SyncProcesState.MoveStart)
        {
            OtherTime -= time;
    ...
    private void SeabedUpdate(float time)
    {
        if (!MoveController.IsIdle) return;
        Singleton<RainbowSeabedController>.Instance.UpdateSeabedBox(CurMsg.Boxs);
        OnSetState?.Invoke(PlayerControllerImp.SyncPipelineState.DoneToIdle);
    }
