...
public static class SelfConfigData
{
    ...
    public static string selfNumber = "16688888884"; // 为11位手机号。 此手机号要加入白名单。  （加入白名单后，验证码固定为手机号的后四位， 登录验证码时通过截取此处的后四位拿取的 ）

    public static string testAppToken = ""; //AppToken （为空时，从接口获取。 不为空时，用此处的值）；

    public static string testLandId = ""; // 土地ID  （为空时，从接口获取。 不为空时，用此处的值） 

    public static int testSceneTypeID = 6; // 6=彩虹沙滩   7=神秘海湾   8=海底星空
