...
public class PersonalDataPanel : BaseUIForm
{
    ...
    void RefreshPersonImgFun()
    {
        ...
        else if (SceneManager.GetActiveScene().name == "Seabed01")
        {
            if (Singleton<RainbowSeabedController>.Instance.Players.ContainsKey(userId))
            {
                picturePath = Singleton<RainbowSeabedController>.Instance.Players[userId].UserInfo.PicUrl;
            }
        }
        if (picturePath != "")
        {
