...
public class UIChat : BaseUIForm
{
    ...
    private void SendMsg(ChatType chatType)
    {
        ...
        {
            var model = Singleton<TreasureModel>.Instance;
             ...
             if (jsonArry != null)
             {
                 ...
                 for (int i = 0; i < jsonArry.Count; i++)
                 {
                     ...
                     if (jo["type"].ToString().Equals("text"))
                     {
                         ...
                         //聊天气泡
                         ShowChatBubble(chatData, chatType);
                     }
                 }
        ...
        ReceiveMessage("OnGetUserInfo", p =>
         {
             ...
             {
                 //聊天气泡
                 ShowChatBubble(m_ChatData, (int)m_ChatType);
                 if (m_ChatType == ChatType.All)
                 {
                     SetChatFoldState(bFold, m_ChatType);
                     SetChatPageInfo(m_ChatType);
    ...
    {
        var allPlayers = controller.Players;
        ulong userId = 0;
        {
            foreach (var player in allPlayers.Values)
            {
                if (player.UserInfo.YunxinAccid.Equals(data.fromAccId))
                {
            ...
            if (controller.Players.TryGetValue(userId, out var imp))
            {
