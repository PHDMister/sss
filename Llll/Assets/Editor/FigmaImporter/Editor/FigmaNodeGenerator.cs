    ...
    public class FigmaNodeGenerator
    {
        ...
        public async Task GenerateNode(Node node, GameObject parent, IList<NodeTreeElement> nodeTreeElements)
        {
            ...
            if (treeElement.actionType != ActionType.None)
            {
                ...
                if (nodeGo == null)
                {
                    ...
                    if (isParentCanvas) root = parentT;
                ...
                if (!isParentCanvas) TransformUtils.SetConstraints(parentT, rectTransform, node.constraints);
                //ImageUtils.SetMask(node, nodeGo); 暂时不需要再prefab根节点添加mask
            }
        ...
        private void AddText(Node node, GameObject nodeGo)
        {
            if (node.type == "TEXT")
            {
                if (useText)
                {
                    var tmp = TMPUtils.GetOrAddTextCompToObject(nodeGo);
                    tmp.text = node.characters;
                    var style = node.style;
                    TMPUtils.ApplyFigmaStyleToTextComp(tmp, style, _importer.Scale);
                    tmp.alignment = TMPUtils.FigmaAlignmentToTextComp(style.textAlignHorizontal, style.textAlignVertical);
                    tmp.fontStyle = TMPUtils.FigmaFontStyleToTextComp(style.textDecoration, style.textCase);
                    tmp.lineSpacing = 1;
                    if (node.fills != null && node.fills.Length >= 1) tmp.color = node.fills[0].color.ToColor();
                    if (node.effects != null && node.effects.Length >= 1)
                    {
                        foreach (var nodeEffect in node.effects)
                        {
                            if (nodeEffect.type.ToLower().Contains("shadow"))
                            {
                                Shadow dShadow = tmp.GetComponent<Shadow>();
                                if (!dShadow) dShadow = tmp.gameObject.AddComponent<Shadow>();
                                dShadow.effectColor = nodeEffect.color.ToColor();
                                dShadow.effectDistance = nodeEffect.offset.ToVector2();
                            }
                            else if (nodeEffect.type.ToLower().Contains("outline"))
                            {
                                Outline outline = tmp.GetComponent<Outline>();
                                if (!outline) outline = tmp.gameObject.AddComponent<Outline>();
                                outline.effectColor = nodeEffect.color.ToColor();
                                outline.effectDistance = nodeEffect.offset.ToVector2();
                            }
                        }
                    }
                ...
                switch (fill.type)
                {
                    case "SOLID":
                        if (tmp != null)
                            ...
                            if (image) image.color = fill.color.ToColor();
                        break;
